...
variable "loadbalancer-pubip" {
    default = "8551-loadbalancer-pubip"
}

variable "loadbalancer" {
    type = map(string) 
    default = {
        name    = "8551-loadbalancer"
        frontend_ip_configuration = "pubip-8551"
    }
}

variable "backend_address_pool" {
    default = "8551-backendaddresspool"
}

variable "loadbalancer_pool_association" {
    default = "8551-loadbalancer-pool-association"
}

variable "loadbalancer_rule" {
    type = map(string) 
    default = {
        name = "8551-loadbalancer-rule"
        protocol = "Tcp"
        frontend_port = 3389
        backend_port = 3389
